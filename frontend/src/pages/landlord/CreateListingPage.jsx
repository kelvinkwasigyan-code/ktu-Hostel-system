...
export default function CreateListingPage() {
  ...
  // Dynamic Room Options & Pricing states
  ...
  // Amenities and photos state
  const [selectedAmenities, setSelectedAmenities] = useState([]);
  // Photos: each entry = { file: File|null, previewUrl: string, storedUrl: string|null }
  // storedUrl is null when Supabase Storage hasn't accepted it yet
  ...
  const handleAddRoomRate = () => {
    setRoomRates([
      ...roomRates,
      { room_type: 'Shared', price_per_semester: '', max_occupancy: '2' }
    ]);
  };
    ...
    try {
      const results = await Promise.all(
        files.map(async (file) => {
          ...
          try {
            // Try to push directly to Supabase Storage
      ...
      if (failed === 0) {
        ...
        toast.success(`${files.length - failed} uploaded to cloud, ${failed} will be retried on submit.`);
      }
    } catch (err) {
      toast.dismiss(loadingToast);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Retry any photos that didn't make it to Supabase Storage yet
          ...
          <div className="container-fluid p-0" style={{ maxWidth: '900px' }}>
            ...
            <form onSubmit={handleSubmit} className="row g-4">
              ...
              <div className="col-md-7">
                <div className="card p-4 border-custom bg-surface rounded-custom d-flex flex-column gap-3">
                  ...
                  <div className="p-3 bg-surface-2 rounded-custom border-custom d-flex flex-column gap-3">
                    <div className="d-flex justify-content-between align-items-center flex-wrap gap-2">
                      ...
                      <button type="button" className="btn btn-sm btn-outline-warning d-flex align-items-center gap-1 py-1" onClick={handleAddRoomRate} style={{ fontSize: '0.78rem' }}>
                ...
                {/* Landlord Contact & Payment Details for Student Payment */}
                  ...
                  <div className="d-flex flex-wrap gap-2 mb-3">
                    {photos.map((p, i) => (
                      <div key={i} className="position-relative" style={{ width: '70px', height: '55px', borderRadius: '6px', overflow: 'hidden' }}>
                        ...
                        {/* Green dot = stored on cloud, yellow dot = pending retry */}
                        <span
                          title={p.storedUrl ? 'Uploaded to cloud' : 'Will retry on submit'}
                          style={{
                    ...
                    {photos.length < 5 && (
                      <label className="d-flex flex-column align-items-center justify-content-center border-custom bg-surface-2 rounded-custom cursor-pointer" style={{ width: '70px', height: '55px', border: '1px dashed var(--border)', opacity: uploadingPhotos ? 0.6 : 1 }}>
                        {uploadingPhotos ? (
                          <div className="spinner-border spinner-border-sm text-warning" role="status" style={{ width: '14px', height: '14px' }} />
                        ) : (
                          <>
                        ...
                        <input type="file" multiple accept="image/*" className="d-none" disabled={uploadingPhotos} onChange={handlePhotoUpload} />
                      </label>
                    )}
                  </div>
                </div>
